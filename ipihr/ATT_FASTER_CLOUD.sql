/* Formatted on 6/29/2026 11:34:06 AM (QP5 v5.362) */
CREATE OR REPLACE PROCEDURE att_faster_cloud (p_fdate         DATE,
                                              p_tdate         DATE,
                                              p_empcode       VARCHAR2,
                                              p_dp_code       VARCHAR2,
                                              o_msg       OUT VARCHAR2)
 ...
 ON (a.checktime = b.checktime AND a.userid = b.userid)
   ...
   */
    ...
    */
    ...
    WHEN MATCHED
    THEN
        UPDATE SET is_holiday = DECODE (b.TYPE, 'HOLIDAY', 1, 0),
                   is_govt_holiday = DECODE (b.TYPE, 'HOLIDAY', 1, 0),
                   remark = NULL,
                   remark_long = NULL;
 ...
 ---------------------------------------------------------------------------------------
  ...
  WHEN NOT MATCHED THEN
    ...
    */
    ...
    ---Updated holiday logic

    UPDATE att_emp_t01
       SET remark =
               CASE
                   -- Govt Holiday (highest priority)
                   WHEN is_holiday = 1 AND is_govt_holiday = 1
                   THEN
                       'Govt Holiday'
                   -- Weekly Holiday
                   WHEN is_holiday = 1 AND NVL (is_govt_holiday, 0) = 0
                   THEN
                       'Weekly Holiday'
                   -- Shifting Holiday
                   WHEN     is_shift = 1
                        AND (is_holiday = 1 OR is_govt_holiday = 1)
                   THEN
                       'Shifting Holiday'
               END,
           status =
               CASE
                   -- Govt Holiday
                   WHEN is_holiday = 1 AND NVL (is_govt_holiday, 0) = 1
                   THEN
                       'G'
                   -- Weekly Holiday
                   WHEN is_holiday = 1 AND NVL (is_govt_holiday, 0) = 0
                   ...
                   -- Shifting Holiday
                   WHEN     is_shift = 1
                        AND (is_holiday = 1 OR NVL (is_govt_holiday, 0) = 1)
                   THEN
                       'H'
               END
     WHERE (is_holiday = 1 OR NVL (is_govt_holiday, 0) = 1);
    ...
    MERGE INTO att_emp_t01 x
         USING (SELECT b.tran_date     attdate
                  FROM holiday_info b
                 WHERE     b.is_govt_holiday = 1    --  b.is_other_holiday = 1
